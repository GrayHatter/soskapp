...
CreateGui:
    ...
    Loop, READ, %ControlSource%
    {
        ...
        {
            Ctl%CtlMax%_X := (match1 + base_x) * GuiScale
            Ctl%CtlMax%_y := (match2 + base_y) * GuiScale
            Ctl%CtlMax%_W := match3 * GuiScale
            Ctl%CtlMax%_H := match4 * GuiScale
            Ctl%CtlMax%_X2 := (match1 + match3 + base_x) * GuiScale
            Ctl%CtlMax%_Y2 := (match2 + match4 + base_y) * GuiScale
        }
